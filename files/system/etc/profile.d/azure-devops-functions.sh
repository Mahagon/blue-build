...
_azure_pipelines_run() {
  ...
  for pipeline_id in "${pipeline_ids[@]}"; do
    ...
    run_id=$(az pipelines run \
      ...
      --branch "$branch" \
      --id "$pipeline_id" \
      --parameters "${type}_NAME=$name" "${type}_VERSION=$tag" \
      --query "id" \
      --output tsv)
...
chart_promote() { _azure_pipelines_run CHART "$1" "$2" 211 267 54 55; }
image_promote()  { _azure_pipelines_run IMAGE "$1" "$2" 210 198 157 158; }
...
if [[ -n "${BASH_VERSION:-}" ]]; then
  ...
  complete -F _chart_subfolders chart_tag chart_promote
  complete -F _image_subfolders image_tag image_promote
fi
